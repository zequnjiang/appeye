...
test('V2-AC-09/11: failed and unsupported enrichment retain the last successful payload and separately report attempt context', () => {
  ...
  try {
    ...
    const failure = store.failEnrichment(
      app.id,
      'permissions',
      'Fixture timeout',
      {
    ...
    const unsupported = store.saveEnrichment(
      app.id,
      'permissions',
      {
        status: 'unsupported',
    ...
    assert.deepEqual(
      history.history.map((item) => item.status),
      ['unsupported', 'failed', 'available'],
    );
...
test('V2-AC-09/11: first failure is not an empty success; successful empty response is distinguishable and updates success time', () => {
  ...
  try {
    ...
    const failure = store.failEnrichment(
      ...
      'Fixture unavailable',
      {
        source: applePrivacyResult.source,
        requestCountry: 'th',
    ...
    const empty = store.saveEnrichment(
      app.id,
      'privacy',
      {
        ...applePrivacyResult,
    ...
    assert.equal(
      store.listEnrichmentHistory(app.id, 'privacy', 1, 1).history[0].status,
      'available',
    );
...
test('V2-AC-09: unknown nested detail/search/review fields survive normalization and all repeated search responses remain inspectable', () => {
  ...
  try {
    const raw = {
      appId: 'fixture.raw.complete',
      title: 'Fixture Raw',
      developer: 'Display Brand',
      developerLegalName: 'Stated Development Entity',
      sellerName: 'Stated Sales Entity',
      ...
      developerEmail: 'fixture@example.invalid',
    ...
    store.saveReviews(app.id, [
      {
        externalId: 'fixture-review',
        text: 'Text',
        raw: { unknownReviewField: { preserved: true } },
      },
    ]);
    assert.deepEqual(store.listReviews(app.id).reviews[0].raw, {
      unknownReviewField: { preserved: true },
    });
  } finally {
    store.close();
  }
