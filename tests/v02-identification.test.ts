...
test('V2-AC-03/04: weak credit/category/name hints and missing descriptions never automatically confirm', () => {
  for (const input of [
    {
      title: 'Credit Points',
      description: 'Earn reward points and keep track of expenses.',
      genre: 'Finance',
    },
    ...
    const result = classifyLoan({
      ...input,
      country: 'ph',
      store: 'google-play',
      externalId: 'fixture.weak',
    });
...
test('V2-AC-04/06: deposit yields and standalone amounts do not become loan APR disclosures', () => {
  for (const input of [depositFixture, amountOnlyFixture]) {
    ...
    assert.ok(
      !result.disclosures.some((item) => item.observedFields.includes('maximumApr')),
      input.title,
    );
  }
});
...
test('V2-AC-06/08: maximum ordinary annual interest is distinct from APR; Thai 15% branch needs explicit annual maximum', () => {
  const input = {
    title: 'Example Personal Loan',
    externalId: 'fixture.th.rate',
    store: 'google-play' as const,
    country: 'th',
    description:
      'Apply for a personal loan of 1000 to 10000 baht. Repayment term: 3 to 12 months. Maximum annual interest rate: 14%. Processing fee: 100 baht.',
  };
  const ordinary = classifyLoan(input);
  assert.ok(
    ordinary.disclosures.some((item) => item.observedFields.includes('maximumInterestRate')),
  );
  assert.ok(!ordinary.disclosures.some((item) => item.observedFields.includes('maximumApr')));
  assert.equal(
    ordinary.disclosures.find((item) => item.ruleId === 'GP-TH-UNREGULATED')?.status,
    'not_observed',
  );
  const apr = classifyLoan({
    ...input,
    description: input.description + ' Maximum annual percentage rate (APR): 18%.',
  });
  assert.ok(apr.disclosures.some((item) => item.observedFields.includes('maximumApr')));
  const unknownPeriod = classifyLoan({
    ...input,
    description:
      'Apply for a personal loan of 1000 to 10000 baht. Repayment term: 3 to 12 months. Interest rate: 14%. Fee: 100 baht.',
  });
  assert.equal(
    unknownPeriod.disclosures.find((item) => item.ruleId === 'GP-TH-UNREGULATED')?.status,
    'conditional',
  );
});
...
test('V2-AC-07/08: Google Play country disclosure rules never become Apple admission or another country requirements', () => {
  ...
  assert.ok(
    apple.disclosures
      .filter((item) => item.ruleId.startsWith('GP-'))
  ...
  for (const country of ['mx', 'ar', 'vn']) {
    ...
    assert.ok(
      result.disclosures
        .filter((item) => /^GP-(TH|PH|ID|PK)-/.test(item.ruleId))
        .every((item) => item.status === 'not_applicable'),
      country,
    );
  }
...
test('V2-AC-08: EWA and revolving credit do not inherit personal-loan disclosure applicability', () => {
  for (const description of [
    ...
    const result = classifyLoan({
      ...facilitatorFixture,
      title: 'Example Credit Service',
      description,
    });
    assert.ok(
      result.disclosures
        .filter((item) => item.ruleId.startsWith('GP-PL-'))
        .every((item) => item.status === 'not_applicable'),
    );
  }
});
...
test('V2-AC-06/10: company roles remain attributed claims and neither display names nor regulator names prove a lender', () => {
  const described = classifyLoan({
    ...
    description:
      ...
      ' Loan provider: Fictional Lender Corporation. Developer legal entity: Fictional Technology Limited. Business name: Fixture Cash. SEC registration: TEST-001. Certificate of Authority: TEST-002.',
  });
  assert.ok(
    described.entities.some(
      (entity) => entity.role === 'loanProvider' && entity.name.includes('Fictional Lender'),
    ),
  );
  assert.ok(
    described.entities.some(
      (entity) =>
        entity.role === 'developerLegalEntity' && entity.name.includes('Fictional Technology'),
    ),
  );
  assert.ok(described.entities.every((entity) => entity.verified === false));
  assert.ok(
    described.entities.every((entity) =>
      described.evidence.some((evidence) => evidence.id === entity.evidenceId),
    ),
  );
  const ambiguous = classifyLoan({
    ...facilitatorFixture,
    description: 'A finance information app. Learn about OJK and SECP.',
    raw: {
      developer: 'Fictional Credit Display Brand',
      sellerName: 'Fictional Publishing Company',
    },
  });
  assert.ok(
    !ambiguous.entities.some(
      (entity) => entity.role === 'loanProvider' || entity.role === 'developerLegalEntity',
    ),
  );
...
test('V2-AC-04/08: explicit denial and provider labels do not create a loan application intent', () => {
  const result = classifyLoan({
    ...facilitatorFixture,
    description:
      'Compare loans. We do not offer or arrange loans. Loan provider: Example Bank. Maximum APR 24%. Loan term 90 to 180 days.',
  });
...
test('V2-AC-06/08: a nearby service fee cannot replace the Thai maximum annual interest rate', () => {
  const result = classifyLoan({
    ...
    country: 'th',
    description:
      'Apply for a personal loan. Maximum annual interest 20% and service fee 5%. Repayment term 90 to 180 days.',
  });
  ...
  assert.equal(
    result.disclosures.find((item) => item.ruleId === 'GP-TH-UNREGULATED')?.status,
    'not_applicable',
  );
});

test('V2-AC-08: EWA-specific disclosure context is applied only to earned wage access', () => {
  const ewa = classifyLoan({
    ...facilitatorFixture,
    description:
      'This is earned wage access (EWA), not a personal loan. Access your already earned wages before payday. Service fee: 2%.',
  });
  const personal = classifyLoan(facilitatorFixture);
  assert.notEqual(
    ewa.disclosures.find((item) => item.ruleId === 'GP-EWA-DISCLOSURES')?.status,
    'not_applicable',
  );
  assert.equal(
    personal.disclosures.find((item) => item.ruleId === 'GP-EWA-DISCLOSURES')?.status,
    'not_applicable',
  );
});

test('V2-AC-06: two adjacent interest/APR values bind to their own labels, and a daily rate never inherits a later annual period', () => {
  const separate = classifyLoan({
    ...facilitatorFixture,
    country: 'th',
    description:
      'Apply for a personal loan. Maximum annual interest 20% and maximum APR 24%. Repayment term 90 to 180 days.',
  });
  assert.ok(
    separate.evidence.some(
      (item) => item.field === 'maximumInterestRate' && item.numericValue === 20,
    ),
  );
  assert.ok(
    separate.evidence.some((item) => item.field === 'maximumApr' && item.numericValue === 24),
  );
  assert.ok(
    !separate.evidence.some((item) => item.field === 'maximumApr' && item.numericValue === 20),
  );
  assert.equal(
    separate.disclosures.find((item) => item.ruleId === 'GP-TH-UNREGULATED')?.status,
    'not_applicable',
  );
  const period = classifyLoan({
    ...facilitatorFixture,
    description:
      'Apply for a loan. Interest is 1% daily, up to 36% annually. Loan term 90 to 180 days.',
  });
  assert.ok(!period.evidence.some((item) => item.numericValue === 1 && item.unit === '%/year'));
});

test('V2-AC-04/06: application approval time after a generic repayment sentence is not a numeric loan repayment term', () => {
  const result = classifyLoan({
    ...facilitatorFixture,
    description:
      'Apply for a personal loan. Maximum annual interest rate 20%. Loan repayment terms are available on our website. Approval within 1 to 2 days.',
  });
  assert.notEqual(result.verdict, 'strong');
  assert.ok(!result.evidence.some((item) => /RepaymentTerm|repaymentTerm/.test(item.field)));
});

test('V2-AC-04: a real lender matchmaking service remains strong when it also offers a loan calculator', () => {
  const result = classifyLoan({ ...facilitatorFixture, description: 'Our loan calculator helps compare costs. We do not provide loans directly; we connect borrowers with lenders. Apply for a personal loan. Maximum APR 24%. Repayment term 90 to 180 days.' });
  assert.equal(result.verdict, 'strong');
  assert.notEqual(result.productType, 'accessory');
});
